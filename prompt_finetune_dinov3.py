...
For each sample in 1_step1_seen.json:
  ...
  3. Select the combo whose output has the highest DINOv3 embedding cosine
     similarity (pooler_output + L2 normalise) with the target
...
RAG_DELTA = 20           # offset applied to range (rag_num)   — must match gen_img.py
...
MAX_SAMPLES    = 100       # only use the first N samples (set to None to use all)
IMG_DIR        = './pairUAV/tours/'
TEST_DIR       = './pairUAV/test/'
GENERATED_DIR  = './outputs/'       # pre-generated images from gen_img.py
OUTPUT_DIR     = './finetune_results_dinov3/'

DINOV3_MODEL = './models/dinov3-vitl16-pretrain-lvd1689m'
...
# ── DINOv3 embedding helpers ─────────────────────────────────────────────

def load_dinov3(model_path: str, device: str):
...
@torch.no_grad()
def get_dino_embedding(processor, dino_model, img_np: np.ndarray,
                       device: str) -> torch.Tensor:
    """Extract L2-normalised pooled embedding from a single RGB uint8 image.

    Returns a 1-D tensor on *device* with unit norm.
    """
    pil_img = Image.fromarray(img_np)  # RGB uint8 → PIL
    inputs = processor(images=pil_img, return_tensors="pt").to(device)
    outputs = dino_model(**inputs)
    pooled = outputs.pooler_output                    # (1, D)
    pooled = F.normalize(pooled, p=2, dim=-1)         # L2 normalise
    return pooled.squeeze(0)                          # (D,)


def compute_dino_similarity(emb1: torch.Tensor, emb2: torch.Tensor) -> float:
    """Cosine similarity between two L2-normalised embeddings (== dot product)."""
    return float(torch.dot(emb1, emb2).cpu())
...
def main():
    ...
    # ── Load DINOv3 model ────────────────────────────────────────────────
    print(f"Loading DINOv3 model from {DINOV3_MODEL} ...")
    dino_processor, dino_model = load_dinov3(DINOV3_MODEL, DEVICE)
    ...
    for idx in range(n_samples):
        ...
        # Pre-compute target DINOv3 embedding (once per sample)
        target_emb = get_dino_embedding(dino_processor, dino_model,
                                        target_img, DEVICE)
        ...
        # ── Compute DINOv3 cosine similarity for each variant against target
        sim_scores = []
        for ci, gen_img in enumerate(gen_images):
            gen_emb = get_dino_embedding(dino_processor, dino_model,
                                         gen_img, DEVICE)
            s = compute_dino_similarity(gen_emb, target_emb)
            sim_scores.append(s)
            print(f"  DINOv3_sim[{ci}] (deg_off={combos[ci][0]:+.0f}, "
                  f"rag_off={combos[ci][1]:+.0f}): {s:.6f}")
        ...
        results_summary.append({
            ...
            'best_dino_sim': best_sim,
            'dino_sim_scores': sim_scores,
        ...
        axes[2, 0].set_title(f"Best (DINOv3_sim={best_sim:.4f})", fontsize=10)
        axes[2, 0].axis('off')
        ...
        # Column 4: DINOv3 similarity bar chart
        ...
        axes[0, 4].set_xlabel('DINOv3 cosine similarity')
        axes[0, 4].set_title('DINOv3 similarity (higher is better)')
    ...
    # Save summary JSON
    summary = {
        'metric': 'DINOv3_cosine_similarity',
    ...
    fig.suptitle(f"MAE Before vs After Prompt Fine-tuning [DINOv3] (deg_delta={DEG_DELTA}, rag_delta={RAG_DELTA})")
