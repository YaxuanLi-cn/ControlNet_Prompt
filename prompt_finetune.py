...
For each sample in 1_step1_seen.json:
  ...
  3. Select the combo whose output has the highest SSIM with the target
...
def compute_ssim(img1: np.ndarray, img2: np.ndarray) -> float:
    """Compute SSIM between two RGB uint8 images (resized to match if needed)."""
...
def main():
    ...
    for idx in range(n_samples):
        ...
        # ── Compute SSIM for each variant against target ─────────────────
        ssim_scores = []
        for ci, gen_img in enumerate(gen_images):
            s = compute_ssim(gen_img, target_img)
            ssim_scores.append(s)
            print(f"  SSIM[{ci}] (deg_off={combos[ci][0]:+.0f}, "
                  f"rag_off={combos[ci][1]:+.0f}): {s:.4f}")

        best_idx = int(np.argmax(ssim_scores))
        best_deg, best_rag = adjusted_vals[best_idx]
        best_ssim = ssim_scores[best_idx]
        ...
        axes[2, 0].set_title(f"Best (SSIM={best_ssim:.4f})", fontsize=10)
        axes[2, 0].axis('off')
        ...
        axes[0, 4].set_xlabel('SSIM')
        axes[0, 4].set_title('SSIM scores')
    ...
    fig.suptitle(f"MAE Before vs After Prompt Fine-tuning (deg_delta={DEG_DELTA}, rag_delta={RAG_DELTA})")
